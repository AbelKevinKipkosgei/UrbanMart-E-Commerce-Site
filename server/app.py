...
class OrderResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            }, 201  # Serialize if using SerializerMixin
    ...
    def post(self):
        ...
        if "username" not in data or "password" not in data:
            return make_response(
                jsonify({"error": "Username and password are required"}), 422
            )
...
class PromoteUserResource(Resource):
    ...
    def post(self, user_id):
        ...
        db.session.commit()
        return make_response(
            jsonify({"message": f"User {user.username} has been promoted to admin."}),
            200,
        )
...
class DemoteUserResource(Resource):
    ...
    def post(self, user_id):
        ...
        db.session.commit()
        return make_response(
            jsonify({"message": f"Admin {user.username} has been demoted to user."}),
            200,
        )
